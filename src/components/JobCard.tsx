...
export default function JobCard({ job }: { job: Job }) {
  return (
    <article className="card-3d group w-full max-w-full min-w-0 overflow-hidden rounded-2xl border border-border bg-surface p-4 sm:p-5 hover:border-accent/50">
      <Link href={`/jobs/${job.id}`} className="block min-w-0">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 flex-1 overflow-hidden">
            <div className="flex min-w-0 items-start justify-between gap-2">
              <h3 className="min-w-0 flex-1 text-sm font-semibold leading-snug text-fg break-words sm:text-base sm:truncate transition-colors group-hover:text-accent">
                {job.title}
              </h3>
      ...
      <div className="mt-3 flex min-w-0 items-center justify-between gap-2 border-t border-border pt-3">
        ...
        <a
          ...
          className="shrink-0 text-xs font-semibold text-accent transition-opacity hover:opacity-70 sm:text-sm"
        >
